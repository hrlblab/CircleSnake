...
def pq_compute_single_core(proc_id, annotation_set, gt_folder, pred_folder, categories):
    ...
    for gt_ann, pred_ann in annotation_set:
        ...
        pan_pred = np.array(
            Image.open(os.path.join(pred_folder, pred_ann["file_name"])), dtype=np.uint32
        )
...
def main():
    ...
    predictionPath = os.environ.get("CITYSCAPES_RESULTS", os.path.join(cityscapesPath, "results"))
    ...
    if not os.path.isfile(args.gtJsonFile):
        printError(
            "Could not find a ground truth json file in {}. Please run the script with '--help'".format(
                args.gtJsonFile
            )
        )
    ...
    if not os.path.isfile(args.predictionJsonFile):
        printError(
            "Could not find a prediction json file in {}. Please run the script with '--help'".format(
                args.predictionJsonFile
            )
        )
