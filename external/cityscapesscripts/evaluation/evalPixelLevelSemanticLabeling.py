...
if "CITYSCAPES_EXPORT_DIR" in os.environ:
    ...
    args.exportFile = os.path.join(
        args.cityscapesPath, "evaluationResults", "resultPixelLevelSemanticLabeling.json"
    )
# Parameters that should be modified by user
args.groundTruthSearch = os.path.join(
    args.cityscapesPath, "gtFine", "val", "*", "*_gtFine_labelIds.png"
)
...
args.colorized = (
    hasattr(sys.stderr, "isatty") and sys.stderr.isatty() and platform.system() == "Linux"
)
...
def getIouScoreForCategory(category, confMatrix, args):
    ...
    labelIds = [
        label.id for label in labels if not label.ignoreInEval and label.id in args.evalLabels
    ]
...
def printConfMatrix(confMatrix, args):
    ...
    for label in args.evalLabels:
        print(
            "\b{text:{fill}>{width}}".format(width=args.printRow + 2, fill="-", text=" "), end=" "
        )
    ...
    for label in args.evalLabels:
        print(
            "\b{text:^{width}} |".format(width=args.printRow, text=id2label[label].name[0]), end=" "
        )
    ...
    for x in range(0, confMatrix.shape[0]):
        ...
        for y in range(0, len(confMatrix[x])):
            ...
            print(
                getColorEntry(matrixFieldValue, args)
                + "\b{text:>{width}.2f}  ".format(width=args.printRow, text=matrixFieldValue)
                + args.nocol,
                end=" ",
            )
        # print prior
        print(
            ...
            + "\b{text:>{width}.4f} ".format(width=6, text=prior)
            + args.nocol
        )
    # print line
    print("\b{text:{fill}>{width}}".format(width=15, fill="-", text=" "), end=" ")
...
def printClassScores(scoreList, instScoreList, args):
    ...
    for label in args.evalLabels:
        ...
        iouStr = (
            getColorEntry(scoreList[labelName], args)
            + "{val:>5.3f}".format(val=scoreList[labelName])
            + args.nocol
        )
        niouStr = (
            getColorEntry(instScoreList[labelName], args)
            + "{val:>5.3f}".format(val=instScoreList[labelName])
            + args.nocol
        )
...
def printCategoryScores(scoreDict, instScoreDict, args):
    ...
    for categoryName in scoreDict:
        if all(label.ignoreInEval for label in category2labels[categoryName]):
            continue
        iouStr = (
            getColorEntry(scoreDict[categoryName], args)
            + "{val:>5.3f}".format(val=scoreDict[categoryName])
            + args.nocol
        )
        niouStr = (
            getColorEntry(instScoreDict[categoryName], args)
            + "{val:>5.3f}".format(val=instScoreDict[categoryName])
            + args.nocol
        )
...
def evaluateImgLists(predictionImgList, groundTruthImgList, args):
    ...
    for i in range(len(predictionImgList)):
        ...
        nbPixels += evaluatePair(
            predictionImgFileName,
    ...
    if not args.quiet:
        ...
        iouAvgStr = (
            getColorEntry(getScoreAverage(classScoreList, args), args)
            + "{avg:5.3f}".format(avg=getScoreAverage(classScoreList, args))
            + args.nocol
        )
        niouAvgStr = (
            getColorEntry(getScoreAverage(classInstScoreList, args), args)
            + "{avg:5.3f}".format(avg=getScoreAverage(classInstScoreList, args))
            + args.nocol
        )
        print("--------------------------------")
    ...
    if not args.quiet:
        ...
        iouAvgStr = (
            getColorEntry(getScoreAverage(categoryScoreList, args), args)
            + "{avg:5.3f}".format(avg=getScoreAverage(categoryScoreList, args))
            + args.nocol
        )
        niouAvgStr = (
            getColorEntry(getScoreAverage(categoryInstScoreList, args), args)
            + "{avg:5.3f}".format(avg=getScoreAverage(categoryInstScoreList, args))
            + args.nocol
        )
        print("--------------------------------")
        print("Score Average : " + iouAvgStr + "    " + niouAvgStr)
    ...
    # write result file
    allResultsDict = createResultDict(
        confMatrix,
        classScoreList,
        classInstScoreList,
        categoryScoreList,
...
def evaluatePair(
    predictionImgFileName, groundTruthImgFileName, confMatrix, instanceStats, perImageStats, args
):
    ...
    if args.evalInstLevelScore:
        ...
        for category in instanceStats["categories"]:
            categoryMasks[category] = np.in1d(
                predictionNp, instanceStats["categories"][category]["labelIds"]
            ).reshape(predictionNp.shape)
    ...
    if args.evalPixelAccuracy:
        ...
        notIgnoredPixels = np.in1d(groundTruthNp, notIgnoredLabels, invert=True).reshape(
            groundTruthNp.shape
        )
        erroneousPixels = np.logical_and(notIgnoredPixels, (predictionNp != groundTruthNp))
        perImageStats[predictionImgFileName] = {}
        perImageStats[predictionImgFileName]["nbNotIgnoredPixels"] = np.count_nonzero(
            notIgnoredPixels
        )
