...
def prepare_training_init(ret, batch):
    ...
    init.update(
        {"ind": torch.cat([torch.full([ct_num[i]], i) for i in range(ct_01.size(0))], dim=0)}
    )
...
def prepare_training_box(ret, batch, init):
    ...
    i_it_4py = torch.cat(
        [snake_decode.get_init(box[i][box_ind[i]][None]) for i in range(batch_size)], dim=1
    )
...
def prepare_training(ret, batch):
    ...
    ct_num = batch["meta"]["ct_num"]
    init.update(
        {"4py_ind": torch.cat([torch.full([ct_num[i]], i) for i in range(ct_01.size(0))], dim=0)}
    )
    init.update(
        {"py_ind": torch.cat([torch.full([ct_num[i]], i) for i in range(ct_01.size(0))], dim=0)}
    )
...
def get_gcn_feature(cnn_feature, img_poly, ind, h, w):
    ...
    gcn_feature = torch.zeros([img_poly.size(0), cnn_feature.size(1), img_poly.size(1)]).to(
        img_poly.device
    )
