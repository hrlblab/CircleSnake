...
from lib.utils.snake import (active_spline, snake_config, snake_decode,
                             snake_gcn_utils)
...
class Evolution(nn.Module):
    ...
    def prepare_testing_init(self, output):
        init = snake_gcn_utils.prepare_testing_init(
            output["detection"][..., :4], output["detection"][..., 4]
        )
        output["detection"] = output["detection"][
            output["detection"][..., 4] > snake_config.ct_score
        ]
    ...
    def forward(self, output, cnn_feature, batch=None):
        ...
        if batch is not None and "test" not in batch["meta"]:
            ...
            ex_pred = self.init_poly(
                self.init_gcn, cnn_feature, init["i_it_4py"], init["c_it_4py"], init["4py_ind"]
            )
        ...
        if not self.training:
            with torch.no_grad():
                ...
                ex = self.init_poly(
                    self.init_gcn, cnn_feature, init["i_it_4py"], init["c_it_4py"], init["ind"]
                )
                ret.update({"ex": ex})
